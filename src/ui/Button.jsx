...
function Button({ children, disabled, to, type, onClick }) {
  ...
  const styles = {
    ...
    round: base + ' py-1 px-2.5 md:px-3.5 md:py-2.5 text-xs',
    secondary:
      'inline-block text-sm rounded-full border-2 border-stone-200 font-semibold uppercase tracking-wide text-stone-400 transition-colors duration-300 hover:bg-stone-300 hover:text-stone-800 focus:bg-stone-300 focus:outline-none focus:ring focus:ring-stone-200 focus:ring-offset-2 focus:text-stone-800disabled:cursor-not-allowed px-4 py-2.5 md:px-6 md:py-4  ',
  };
  if (to)
    return (
